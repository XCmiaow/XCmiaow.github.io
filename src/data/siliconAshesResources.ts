...
export type ResourceTemplate = {
  slug: string;
  category: Extract<ResourceCategory, 'templates' | 'checklists'>;
...
type ResourceRegistryEntry = {
  ...
  zh: ResourceIndexItem;
  en: ResourceIndexItem;
};

const templateRegistryEntries: ResourceRegistryEntry[] = resourceTemplates.map((template) => ({
  id: `template:${template.slug}`,
  category: template.category,
  zh: {
    title: template.zh.title,
    summary: template.zh.summary,
    href: `/silicon-ashes/resources/${template.slug}/`,
    meta: [...template.zh.meta],
  },
  en: {
    title: template.en.title,
    summary: template.en.summary,
    href: `/en/silicon-ashes/resources/${template.slug}/`,
    meta: [...template.en.meta],
  },
}));

export const resourceRegistry: readonly ResourceRegistryEntry[] = [
  {
    id: 'course-hub',
  ...
  {
    id: 'feed',
    category: 'wechat-extensions',
    zh: {
      title: '公众号入口',
      summary: '关注「硅基余烬」，获取最新更新。',
      href: '/silicon-ashes/feed/',
      meta: ['公众号', '更新源'],
    },
    en: {
      title: 'WeChat entry',
      summary: 'Follow silicon-embers for new updates.',
      href: '/en/silicon-ashes/feed/',
      meta: ['WeChat', 'Updates'],
    },
  },
  {
  ...
  ...templateRegistryEntries,
];

const groupCopy: Record<ResourceIndexLang, Record<ResourceCategory, { title: string; summary: string }>> = {
  zh: {
    'course-materials': { title: '课程资料', summary: '课程作为资源子模块保留，入口集中在这里。' },
    templates: { title: '模板', summary: '把模糊任务变成可执行、可复用的工作入口。' },
    checklists: { title: '清单', summary: '用于采纳 AI 输出之前的人工验收。' },
    'workflow-tools': { title: '工具流', summary: '课程和日常实践里会反复用到的工具页面。' },
    'wechat-extensions': { title: '公众号延伸', summary: '最新更新在公众号，网站负责索引和资料沉淀。' },
  },
  en: {
    'course-materials': {
    ...
    templates: { title: 'Templates', summary: 'Turn vague work into executable, reusable task entries.' },
    checklists: { title: 'Checklists', summary: 'Human review before accepting AI output.' },
    'workflow-tools': { title: 'Workflow tools', summary: 'Tool pages reused across the course and daily practice.' },
    'wechat-extensions': {
...
const templateEntryCounts = new Map<string, number>();
for (const entry of resourceRegistry) {
  if (!entry.id.startsWith('template:')) continue;
  const slug = entry.id.slice('template:'.length);
  templateEntryCounts.set(slug, (templateEntryCounts.get(slug) ?? 0) + 1);
}
for (const template of resourceTemplates) {
  if (templateEntryCounts.get(template.slug) !== 1) {
    throw new Error(`Resource template ${template.slug} must appear exactly once in resourceRegistry`);
  }
}
...
export const getResourceIndexGroups = (lang: ResourceIndexLang): ResourceIndexGroup[] =>
  resourceCategories.map((category) => ({
    id: category,
    ...groupCopy[lang][category],
    items: resourceRegistry.filter((entry) => entry.category === category).map((entry) => ({ ...entry[lang] })),
  }));
